...
(function(){
	...
	async function loadFallback(){
		...
		try {
			const res = await fetch(`/admin/api/session/patient/${id}/hearing-aids`, { headers: { 'Accept':'application/json' }, credentials: 'same-origin' });
	...
	<div class="row g-3" style="margin-top:32px;">
		<div class="col-12">
			...
			@include('results.hearing-aid-results')
...
(function(){
	...
	async function load(){
		...
		try {
			const res = await fetch("{{ url('/admin/api/patient-records') }}/"+encodeURIComponent(id), { headers: { 'Accept':'application/json' } });
	...
	async function loadHearingAids() {
		...
		try {
			const apiUrl = `/admin/api/session/patient/${id}/hearing-aids`;
	...
	window.deleteHearingAid = async function(hearingAidId) {
		...
		try {
			const response = await fetch(`/admin/api/session/patient/${id}/hearing-aids/${hearingAidId}`, {
				method: 'DELETE',
				headers: {
					'X-CSRF-TOKEN': CSRF,
					'Accept': 'application/json'
	...
	window.deleteServiceResult = async function(serviceKey, resultId){
		...
		try {
			const res = await fetch(`/admin/api/session/patient/${id}/services/${encodeURIComponent(serviceKey)}/${encodeURIComponent(resultId)}`, {
				method: 'DELETE',
