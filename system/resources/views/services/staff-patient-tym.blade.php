...
@section('title', 'Tympanometry | Kamatage Hearing Aid')
...
<div class="container-fluid" style="padding:0 24px;">
  ...
  <div style="margin-top:8px; margin-bottom:18px;" class="d-flex justify-content-between flex-wrap align-items-center">
    <div>
      <h4 style="font-weight:bold; margin-bottom:4px;">Tympanometry</h4>
      <div class="text-muted" style="font-size:.95rem;">Record tympanogram type per ear.</div>
    </div>
  ...
  <div class="row g-3">
      ...
      <div class="card shadow-sm" style="border:1px solid #e2e8f0; border-radius:10px;">
        <div class="card-body">
          ...
          <ul class="list-group patient-menu">
            ...
            <a href="{{ route('staff.patient.record.details.tym', ['id' => $id]) }}" class="list-group-item d-flex align-items-center active text-decoration-none">
    ...
    <!-- Right panel: Tympanometry form -->
    <div class="col-lg-8">
      <div class="card shadow-sm" style="border:1px solid #e2e8f0; border-radius:10px;">
        <div class="card-body">
          <form id="tymForm" method="POST" action="{{ route('staff.patient.service.session.save', ['id' => $id, 'service' => 'tym']) }}" novalidate>
            @csrf
            ...
            <div class="row g-3 mb-3">
              <div class="col-md-6">
                <label for="tym_date_taken" class="form-label fw-semibold">Date Taken</label>
                <input type="date" id="tym_date_taken" name="date_taken" class="form-control">
              </div>
            </div>
            <!-- Notes -->
            <div class="mb-3">
              <label for="tym_notes" class="form-label fw-semibold">Notes</label>
              <textarea id="tym_notes" name="tym_notes" class="form-control" rows="4" placeholder="Enter any additional observations or recommendations..."></textarea>
            </div>
...
<style>
  ...
  /* Smaller font for Right/Left labels */
  .tym-table tbody th { font-size: 0.85rem; font-weight: 600; }
  .tym-table tbody td { padding-top:.4rem; padding-bottom:.4rem; }
  .tym-table input.form-control { height: 2.1rem; }
</style>
@endpush
